...
import { API_BASE_URL } from '~/utils/req'
...
onMounted(() => {
  ...
  if (isDirectAccess.value) {
    ...
    if (password) {
      access_pass.value = encryptText(password);
      handlePass();
    }
  }
});
...
function checkDirectAccess() {
  isDirectAccess.value = Object.keys(router.currentRoute.value.query).includes('password');
}
function encryptText(data: string) {
  const ciphertext = CryptoJS.AES.encrypt(data, '2b7e151628aed2a6abf7158809cf4f3c').toString();
  console.log(ciphertext)
  return ciphertext
}
function decryptText(data: string) {
  const bytes = CryptoJS.AES.decrypt(data,'2b7e151628aed2a6abf7158809cf4f3c');
...
onMounted(async () => {
  ...
  $fetch(`${API_BASE_URL}/api/pass`).then(res => {
...
<template>
  ...
  <UModal v-model="isOpen" v-if="!isDirectAccess" prevent-close>
  ...
  <UContainer class="flex h-full w-full overflow-y-auto">
    <div class="w-48 flex flex-col transition-all z-10 mr-2 mobileBar" :class="{hide:hideTabBar}">
      ...
      <div class="flex mb-1 max-sm:mb-10 max-sm:mx-2" v-show="!hideTabBar">
        ...
        <UButton class="ml-auto" variant="soft" @click.passive.stop="handleNew" style="background-color: dimgray">
          <div  style="color: aliceblue"><!--class="line-clamp-1"-->
            {{ $t('new_chat') }} +
          </div>
        </UButton>
      </div>
    ...
    <div class="flex flex-col w-full">
      <div ref="el" class="py-4 h-full overflow-y-auto pt-24 scrollbar-hide" >
        ...
        <ul class="flex flex-col space-y-2">
          ...
          <template v-for="(i,index) in history" :key="index">
            <li v-if="i.role === 'assistant'" id="reply" class="slide-top" style="background-color: #C0C4CC">
            ...
            <li v-else id="send" class="slide-top" style="background-color: #E4E7ED;color: black">
              {{ i.content }}
            </li>
          </template>
        </ul>
      ...
      <div class="space-y-1 flex flex-col">
        <USelectMenu class="self-center mt-1" v-model="selectedModel" :options="models" style="color: white;background-color: #C0C4CC" option-attribute="name"/>
        ...
        <div class="flex items-end">
          ...
          <UTooltip :text="$t('add_image') + '/' + $t('support_paste')"
            ...
            <UInput id="image" type="file" class="hidden" accept="image/png,image/jpeg,image/webp,image/heic,image/heif"
                    multiple @change="handleImageAdd" />
            <UButton class="m-1" @click="handleImage" :color="upImages.length?'primary':'gray'"
                     icon="i-heroicons-photo"/>
          </UTooltip>
          <UButton @click="handleReq($event, selectedModel)" :disabled="loading" class="m-1" style="background-color: #303133;color: ghostwhite">
            {{ $t('send') }}
          </UButton>
        </div>
      </div>
...
.card-focus {
  @apply ring-2 border-l-amber-400 dark:border-b-white
}
